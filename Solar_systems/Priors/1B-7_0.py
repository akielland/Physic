...
class PlanetOrbits():
    ...
    def generate_orbit(self, time=3600*24*365*60, N=10):
        # N = int(time/dt)
        dt = time/N
        print(dt/3600)
    ...
    # def RHS(r_v):
    #         vel[:,0,t+1] =  - (G*star_m * pos[:,0,t] / np.sqrt(pos[:,0,t]**2 + pos[:,1,t]**2)**3)
    #         vel[:,1,t+1] =  - (G*star_m * pos[:,1,t] / np.sqrt(pos[:,0,t]**2 + pos[:,1,t]**2)**3)
    #         pos[:,0,t+1] = vel[:,0,t+1]
    #         pos[:,1,t+1] = vel[:,1,t+1]
    #     dv = self.G*self.mass_star_SI * r /np.linalg.norm(r)^2
    #     dr = 
    #     drdv = np.array([dv, dr])
    #     return drdv
    ...
    # def forward_euler(self, y, dt):
    #     drdt = RHS(y)*dt
    #     return drdt

    # def step(y, dt, method=RK4):
    #     dydt = method(y, dt)        #, param
    #     return y + dydt*dt    

    # def integration(y0):
    #     # initialize solution vector
    #     y = np.zeros((4,N))
    #     # set initial conditions and initial divergence
    #     y[:,0] = y0
    #     for i in range(N-1):
    #         y[:,i+1] = 
